...
# merge all mapping in a single dataframe for all tissue
merge_umap_cluster_alltissue = data.frame()
for( current_tissue in names( PATH_CLUSTER_MAPPING_FILE)){
  current_new_df = merge_umap_cluster_list[[ current_tissue]]
  current_new_df$tissue = current_tissue
  merge_umap_cluster_alltissue = rbind( merge_umap_cluster_alltissue, current_new_df)
}
...
# Load the table with both Clonotype and Seurat analysis information, restrincint g the information to the filtered cells
clonotype_and_seurat_data_df = read.table( PATH_CLONOTYPE_AND_SEURAT_DATA_FILE, header=TRUE, row.names = 1, sep="\t")
