...
# Identify marker genes one cluster versus all others
...
# Identify marker genes by pairs of clusters
pairwise_markers = data.frame()
for( cluster1 in levels( Idents( sc10x.rna.seurat))){
  for( cluster2 in levels( Idents( sc10x.rna.seurat))){
    if( cluster1 != cluster2){
      current_pairwise_markers = FindMarkers( object          = sc10x.rna.seurat,
                                              ident.1         = cluster1,
                                              ident.2         = cluster2,
                                              test.use        = FINDMARKERS_METHOD,
                                              only.pos        = FINDMARKERS_ONLYPOS,
                                              ...
                                              logfc.threshold = FINDMARKERS_LOGFC_THR,
                                              verbose         = .VERBOSE);
      current_pairwise_markers$cluster1 = rep( cluster1, nrow( current_pairwise_markers))
      current_pairwise_markers$cluster2 = rep( cluster2, nrow( current_pairwise_markers))
      current_pairwise_markers$gene = row.names( current_pairwise_markers)
      pairwise_markers = rbind( pairwise_markers, current_pairwise_markers)
    }
  }
}
