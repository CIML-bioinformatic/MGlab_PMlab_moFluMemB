...
sc10x.rna.seurat = subset( sc10x.rna.seurat, cells = intersect( heavy_and_light_chains_df$cell.bc, Cells( sc10x.rna.seurat)))
...
if( !exists( "current_tissue")){
  CLUSTER_PAIRWISE_MARKER_GENES_DF = data.frame()
  for( tissue in names( PATH_CLUSTER_PAIRWISE_MARKER_GENES_PER_TISSUE)){
    current_table = read.table( PATH_CLUSTER_PAIRWISE_MARKER_GENES_PER_TISSUE[[ tissue]], sep="\t", header=TRUE)
    current_table$tissue = rep( tissue, nrow( current_table))
    CLUSTER_PAIRWISE_MARKER_GENES_DF = rbind( CLUSTER_PAIRWISE_MARKER_GENES_DF, current_table)  
  }
}else{
  CLUSTER_PAIRWISE_MARKER_GENES_DF = read.table( PATH_CLUSTER_PAIRWISE_MARKER_GENES_PER_TISSUE[[ current_tissue]], sep="\t", header=TRUE)
  CLUSTER_PAIRWISE_MARKER_GENES_DF$tissue = rep( current_tissue, nrow( CLUSTER_PAIRWISE_MARKER_GENES_DF))
}
...
# Add metadata on V-segment exact (heavy and light chains)
cell_vsegment_exact_heavy_names = heavy_and_light_chains_df[ Cells( sc10x.rna.seurat), "v.segment.exact.heavy"]
sc10x.rna.seurat = AddMetaData( object = sc10x.rna.seurat, 
                                metadata = cell_vsegment_exact_heavy_names, 
                                col.name = "v.segment.exact.heavy")
cell_vsegment_exact_light_names = heavy_and_light_chains_df[ Cells( sc10x.rna.seurat), "v.segment.exact.light"]
sc10x.rna.seurat = AddMetaData( object = sc10x.rna.seurat, 
                                metadata = cell_vsegment_exact_light_names, 
                                col.name = "v.segment.exact.light")
...
# -- retrieve the large clonotypes from tissues separatly
tissue_large_clonotype_set = unique( row.names( which( table( cell_clonotype_names, cell_tissue) >= LARGE_CLUSTER_THRESHOLD, arr.ind = TRUE)))
if( length( tissue_large_clonotype_set) > 0){
  large_clonotype_set = unique( c (global_large_clonotype_set, tissue_large_clonotype_set))
  
  if( exists( "current_tissue") && current_tissue %in% names( PATH_SEURAT_FILTERED_OBJECT_BY_TISSUE)){
    cat("<b> or with more than", LARGE_CLUSTER_THRESHOLD, "cells in", current_tissue,".</b>")
  } else{
    cat("<b> or with more than", LARGE_CLUSTER_THRESHOLD, "cells in at least one tissues.</b>")
  }
}else{
  large_clonotype_set = global_large_clonotype_set
  cat("<b> (No cluster with more than", LARGE_CLUSTER_THRESHOLD, "cells in this tissue(s))</b>")
}
cat("<BR><BR>")

# --build the large clonotype factor for the Seurat object
...
Idents( sc10x.rna.seurat) = "v.segment.exact.heavy"
clonotype_seurat_df$v.segment.exact.heavy = Idents( sc10x.rna.seurat)[ row.names( clonotype_seurat_df)]
...
Idents( sc10x.rna.seurat) = "v.segment.exact.light"
clonotype_seurat_df$v.segment.exact.light = Idents( sc10x.rna.seurat)[ row.names( clonotype_seurat_df)]
...
# export some specific data required for other scientific analysis
if( exists( "current_tissue") && current_tissue == "LG"){
  cluster_0_1_large_clusters_df = clonotype_seurat_df[ which( clonotype_seurat_df$seurat_clusters %in% c(0,1) & !is.na( clonotype_seurat_df$large.clonotype.similarity.symbol)), ]
  write.table( cluster_0_1_large_clusters_df, file = file.path( PATH_ANALYSIS_OUTPUT, paste0( "clonotype_and_seurat_merged_data_", current_tissue, "_cluster_0_1_largeclonotype.tsv")),
               sep="\t", col.names = NA, quote = FALSE)
}
