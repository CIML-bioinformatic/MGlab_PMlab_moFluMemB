...
print( ggplot( large_clonotype_sample_wide) +
         ...
         theme_minimal() +
         ggtitle( "Percentage of cell in sample for each clonotype")
)
...
print( ggplot( large_clonotype_cluster_wide) +
         ...
         theme_minimal() + 
         ggtitle( "Percentage of cell in cluster for each clonotype")
)
...
if( exists( "current_tissue") && current_tissue == "LG"){
  ...
  cluster_markers_clonotype_expression_df = GetAssayData( sc10x.rna.seurat, assay="RNA", slot="data")[ cluster_marker_genes, ]
  ...
  # -- get the expression of selected cells on marker genes
  cluster_markers_clonotype_expression_df = GetAssayData( sc10x.rna.seurat, assay="RNA", slot="data")[ cluster_marker_genes, current_clonotype_cells]
  ...
  for( current_clonotype in levels( clonotype_seurat_df$large.clonotype.similarity.symbol)){
    if( !is.na( current_clonotype)){
      ...
      if( length( cell_in_cluster_0) > 0 && length( cell_in_cluster_1) > 0){
        cluster_markers_clonotype_expression_cluster_0_df = as.data.frame( GetAssayData( sc10x.rna.seurat, assay="RNA", slot="data")[ cluster_marker_genes, cell_in_cluster_0])
        cluster_markers_clonotype_expression_cluster_0_df$mean = apply( cluster_markers_clonotype_expression_cluster_0_df, 1, mean)
        cluster_markers_clonotype_expression_cluster_1_df = as.data.frame( GetAssayData( sc10x.rna.seurat, assay="RNA", slot="data")[ cluster_marker_genes, cell_in_cluster_1])
  ...
  # Get the expression of selected cells on marker genes
  cluster_pairwise_markers_clonotype_expression_df = GetAssayData( sc10x.rna.seurat, assay="RNA", slot="data")[ cluster_pairwise_marker_genes, ]
  ...
  for( current_clonotype in levels( clonotype_seurat_df$large.clonotype.similarity.symbol)){
    if( !is.na( current_clonotype)){
      ...
      # Get the expression of selected cells on marker genes
      cluster_pairwise_markers_clonotype_expression_df = GetAssayData( sc10x.rna.seurat, assay="RNA", slot="data")[ cluster_pairwise_marker_genes, clonotype_cells]
      ...
      # Show in datatable the expression of cells
      cluster_pairwise_markers_clonotype_expression_cluster_0_df = GetAssayData( sc10x.rna.seurat, assay="RNA", slot="data")[ cluster_pairwise_marker_genes, row.names( annotation_cell)[ which( annotation_cell$Cell.cluster == 0)]]
      cluster_pairwise_markers_clonotype_expression_cluster_1_df = GetAssayData( sc10x.rna.seurat, assay="RNA", slot="data")[ cluster_pairwise_marker_genes, row.names( annotation_cell)[ which( annotation_cell$Cell.cluster == 1)]]
...
# If we are looking at data with multiple tissue, plot the distribution of the Heavy chains for each tissue and each mouse
# If we are looking at data with a single tissue, plot the distribution of the Heavy chains for each cluster and each mouse
if( length( unique( clonotype_seurat_df$tissue)) > 1){
  ...
  print( ggplot( cluster_mouse_isotype_df) +
           ...
           xlab( "Isotype") + ylab( "Percentage of cells") + ggtitle( paste( "Distribution of cells along isotypes and clusters on tissue", current_tissue)) +
           theme_classic()
  )
  
  print( htmltools::tagList( datatable( as.matrix( cluster_mouse_isotype_df), caption=paste( "Distribution of cells along isotypes and clusters on tissue", current_tissue))))
  write.table( cluster_mouse_isotype_df,
               file = file.path( PATH_ANALYSIS_OUTPUT, paste0( SAMPLE_NAME, "_distribution_cells_HEAVYchain_isotypes_", current_tissue, ".csv")),
               sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
}
...
# If we are looking at data with multiple tissue, plot the distribution of the Light chains for each tissue and each mouse
...
if( length( unique( clonotype_seurat_df$tissue)) > 1){
  ...
  print( htmltools::tagList( datatable( as.matrix( cluster_mouse_isotype_df), caption=paste( "Distribution of cells along isotypes and clusters on tissue", current_tissue))))
  write.table( cluster_mouse_isotype_df,
               file = file.path( PATH_ANALYSIS_OUTPUT, paste0( SAMPLE_NAME, "_distribution_cells_LIGHTchain_isotypes_", current_tissue, ".csv")),
               sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
}
...
if( length( unique( clonotype_seurat_df$tissue)) > 1){
  ...
  print( ggplot( cluster_mouse_cdr3length_df) +
           ...
           xlab( "Isotype") + ylab( "Percentage of cells") + ggtitle( paste( "Distribution of cells along CDR3 length on tissue", current_tissue)) +
           theme_classic()
  )
  
  # Print dataframe as datatable
  print( htmltools::tagList( datatable( as.matrix( cluster_mouse_cdr3length_df), caption=paste( "Distribution of cells along CDR3 length on tissue", current_tissue))))
  
  # Export dataframe to file
  cdr3.length.heavy.file_path=file.path( PATH_ANALYSIS_OUTPUT, paste0( SAMPLE_NAME, "_CDR3_length_heavy_stats_mouse_", current_tissue, ".csv"))
