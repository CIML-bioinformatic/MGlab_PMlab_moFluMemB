...
# Plot expression values of marker genes (TODO: message if list empty)
# and export the same plot to SVG files in a specific folder
dir.create( file.path( PATH_ANALYSIS_OUTPUT, "FeaturePlots"), showWarnings = FALSE)
...
resNULL=lapply(names(topMarkers), function(clusterName)
{
  ...
  markersPanels = lapply( topMarkers[[clusterName]][["gene"]], function(featuresName) 
    { 
      marker_display_plot = FeaturePlot( sc10x.rna.seurat, features = featuresName, reduction = ifelse(exists("useReduction"), useReduction, "umap"), order = TRUE) +
                                  theme( axis.title.x = element_blank(),
                                         axis.title.y = element_blank(),
                                         legend.position = "none");

      marker_svg_plot = FeaturePlot( sc10x.rna.seurat, features = featuresName, reduction = ifelse(exists("useReduction"), useReduction, "umap"), order = TRUE)
      ggsave( filename = file.path( PATH_ANALYSIS_OUTPUT, "FeaturePlots", paste0( "Tissue_", CURRENT_TISSUE, "_Cluster_", clusterName, "_FeaturePlotMarker_", featuresName, "_", useReduction, ".svg")),
              plot = marker_svg_plot, width = 5, height = 5)
      
      return( marker_display_plot)
    });
...
resNULL = lapply( names( MONITORED_GENES), function(monitoredGroup)
{
  ...
  expressionPlots = lapply( MONITORED_GENES[[monitoredGroup]], function(featuresName)
  {
    ...
    tryCatch( 
      {
        monitored_svg_plot = FeaturePlot( sc10x.rna.seurat, features = featuresName, reduction = ifelse(exists("useReduction"), useReduction, "umap"), order = TRUE)
        ggsave( filename = file.path( PATH_ANALYSIS_OUTPUT, "FeaturePlots", paste0( "Tissue_", CURRENT_TISSUE, "_Group_", monitoredGroup, "_FeaturePlotMonitored_", featuresName, "_", useReduction, ".svg")),
              plot = monitored_svg_plot,
              width = 5, height = 5)
      },
      error=function(e){return(paste( "GGsave : ", conditionMessage(e)))}); # In case gene name could not be found in object (should have been removed earlier anyway)
