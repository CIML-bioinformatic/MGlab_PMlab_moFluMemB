...
library( funr)
if( exists( "snakemake")){
  cat("<\nINFO : Working in snakemake environment.")
  WORKING_DIR = snakemake@scriptdir
}else{
  cat("\nINFO : Working in script mode.")
  WORKING_DIR = tryCatch( 
    {
      dirname( sys.script())
    },
    error = function( cond){
      cat("...but no path to script found. Using R working dir.")
      return( getwd())
    }
  )
}
...
rmarkdown::render( input = file.path( WORKING_DIR, paste0( PROJECT_SHORT_NAME, "_", ANALYSIS_STEP_NAME, ".Rmd")),
                     output_dir = PATH_ANALYSIS_OUTPUT,
                     output_file  = paste0( PROJECT_SHORT_NAME, "_", ANALYSIS_STEP_NAME, ".html"),
                     quiet = FALSE)  
