...
custom.rna.seurat <- RunPCA( object   = custom.rna.seurat, 
                           features = VariableFeatures( custom.rna.seurat),
                           npcs     = PCA_NPC,
                           verbose  = .VERBOSE);
...
custom.rna.seurat <- FindNeighbors(object  = custom.rna.seurat, 
                                   dims    = 1:30,
                                   verbose = .VERBOSE);

custom.rna.seurat <- FindClusters(object              = custom.rna.seurat, 
                                  # reduction.type     = "pca", 
                                  resolution         = 0.5, 
...
clusterCount = as.data.frame( table( ClusterNumber = custom.rna.seurat[[ "seurat_clusters" ]]), responseName = "CellCount");
...
cellsData = cbind( "Cell" = colnames( custom.rna.seurat), # cell names from rownames conflict with search field in datatable
                   ...
                   "percent.mito" = as.numeric(format(custom.rna.seurat[["percent.mito", drop = TRUE]], digits = 5)),
                   "percent.ribo" = as.numeric(format(custom.rna.seurat[["percent.ribo", drop = TRUE]], digits = 5)),
                   "Cluster" = Idents( custom.rna.seurat));
...
# ---------------------------------------------------------------
# ---------------------------------------------------------------
# Study the relationship between clusters and quality indicators
# ---------------------------------------------------------------
# ---------------------------------------------------------------

## @knitr analyze_cluster_quality

cellsData$Plate = custom.rna.seurat$plate[ row.names( cellsData)]
cellsData$Plate = factor( cellsData$Plate, levels = paste0( "plate", seq( 1, length( unique( cellsData$Plate)),1)))

# Look at the number of cells by plates
ggplot( cellsData %>% group_by( Plate) %>% summarise( cell.number = length( nFeature_RNA)))  + 
  geom_bar( aes( x=Plate, y=cell.number, fill = Plate), stat = "identity") +
  theme_classic() + theme(  legend.position = "None", axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle( "Number of cell by plate")

# Look at the number of cells by plates and clusters
ggplot( cellsData %>% group_by( Plate, Cluster) %>% summarise( cell.number = length( nFeature_RNA)))  + 
  geom_bar( aes( x=Plate, y=cell.number, fill = Cluster), stat = "identity") +
  theme_classic() + theme( axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle( "Number of cell by plate and cluster")

# Look at the chi2 statistics between number of cells against cluster by plates
cluster_plate_wide_df = as.data.frame.matrix( table( cellsData$Cluster, cellsData$Plate))
chisq = chisq.test(  cluster_plate_wide_df[ , ! names( cluster_plate_wide_df) %in% c( "plate7", "plate8", "plate19", "plate20")])

datatable( t( cluster_plate_wide_df), caption = "Number of cells for each plate and clusters")
corrplot::corrplot(chisq$residuals, is.cor = FALSE, main = "Chi2 residual of cell number between clusters and plates")


# Make a bloc of 4 plots to show the number of feature by plates and clusters

ggplot( cellsData) + 
  geom_violin( aes( x= Plate, y= nFeature_RNA, fill = Plate)) + 
  theme_classic() + theme( legend.position = "None", axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle( "Distribution of the number of \nfeature per cells along plates") +

ggplot( cellsData) + 
  geom_boxplot( aes( x= interaction( Plate, Cluster), y= nFeature_RNA, fill = Plate)) + 
  theme_classic() + theme( legend.position = "None", axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle( "Distribution of the number of \nfeature per cells along plates and clusters") +

ggplot( cellsData %>% group_by( Plate) %>% summarise( nFeature_RNA_median = median( nFeature_RNA))) + 
  geom_bar( aes( x=Plate, y=nFeature_RNA_median, fill = Plate), stat = "identity") +
  theme_classic() + theme( legend.position = "None", axis.text.x = element_text(angle = 45, hjust = 1))+
  ggtitle( "Median number of feature \nby plate") +

ggplot( cellsData %>% group_by( Plate, Cluster) %>% summarise( nFeature_RNA_median = median( nFeature_RNA)))  + 
  geom_bar( aes( x=Plate, y=nFeature_RNA_median, fill = Cluster), stat = "identity") +
  theme_classic() + theme( axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle( "Median number of feature \nby plate and cluster")

ggplot( cellsData) + 
  geom_point( aes( x = nFeature_RNA, y=percent.mito, col=Cluster)) +
  theme_classic() +

ggplot( cellsData) + 
  geom_point( aes( x = nFeature_RNA, y=percent.ribo, col=Cluster)) +
  theme_classic() +

ggplot( cellsData) + 
  geom_boxplot( aes( x = Cluster, y=percent.mito, col=Cluster)) +
  theme_classic() +
  
ggplot( cellsData) + 
  geom_boxplot( aes( x = Cluster, y=percent.ribo, col=Cluster)) +
  theme_classic()
...
pcaClustersList = apply( combn( 1:PCA_PLOTS_NBDIMS, 2), 2, function(dims) 
          {
                    ( DimPlot( object=custom.rna.seurat, reduction="pca", dims = dims) + 
...
pcaUmisList = apply( combn( 1:PCA_PLOTS_NBDIMS, 2), 2, function(dims) { ( FeaturePlot( custom.rna.seurat, feature = "nCount_RNA", reduction = "pca", dims = dims) +
...
pcaGenesList = apply( combn( 1:PCA_PLOTS_NBDIMS, 2), 2, function(dims) { ( FeaturePlot( custom.rna.seurat, feature = "nFeature_RNA", reduction = "pca", dims = dims) + theme( legend.position = "none", 
...
relationToPC = suppressWarnings( cbind( 
  stack( as.data.frame( Embeddings( custom.rna.seurat, reduction = "pca")[ rownames( custom.rna.seurat[[]]), paste0( "PC_", 1:PCA_PLOTS_NBDIMS) ])),
  custom.rna.seurat[[ c( "nCount_RNA", "nFeature_RNA") ]],
  Cluster = Idents( custom.rna.seurat)));
...
pcaLoadingsList = apply( combn( 1:PCA_PLOTS_NBDIMS, 2), 2, function(dims)
{
  ...
  loadingsMatrix = Loadings( custom.rna.seurat, reduction = "pca")[ , namesPC ];
...
custom.rna.seurat = RunTSNE( custom.rna.seurat, dims = 1:DIMREDUC_USE_PCA_NBDIMS);
custom.rna.seurat = RunUMAP( custom.rna.seurat, dims = 1:DIMREDUC_USE_PCA_NBDIMS);
...
dimReducData = cbind(as.data.frame(Embeddings(custom.rna.seurat, reduction = ifelse(exists("useReduction"), useReduction, "umap"))),
                     Cluster = as.numeric(as.character(Idents( custom.rna.seurat))));
...
plotDimReduc = plot_ly(dimReducData, 
                       ...
                       color = ~Cluster,
                       colors = hue_pal()( nlevels( Idents( custom.rna.seurat))),
                       height = 600) %>%
...
Idents( custom.rna.seurat) = "MULTI_Sample"

dimReducData = cbind(as.data.frame(Embeddings(custom.rna.seurat, reduction = ifelse(exists("useReduction"), useReduction, "umap"))),
                     MULTI_Sample = as.character(Idents( custom.rna.seurat)));
...
print( suppressWarnings( DimPlot( custom.rna.seurat, reduction = ifelse(exists("useReduction"), useReduction, "umap"), label=TRUE, label.size = 8)) + 
         theme( axis.title.x = element_blank(),
                axis.title.y = element_blank(),
                legend.position = "none", 
                plot.margin = margin(0, 0, 0, 0, "cm")));
...
markers = FindAllMarkers( object          = custom.rna.seurat,
...
#htmltools::tagList( ggplotly( DoHeatmap(object = custom.rna.seurat, features = topMarkersDF[["gene"]])));

# Get the matrix of expression and associated clusters from Seurat object 
expMat = as.matrix( GetAssayData( custom.rna.seurat));
clusterID = Idents( custom.rna.seurat);
...
hoverTextMarkers[] = paste( paste( "Cell ID:", custom.rna.seurat[["numID", drop = FALSE]][colnames( expMat)[col( expMat)],]),
                            paste( "Value:", expMat),
...
heatmapPlot = iheatmap(expMat, 
                       ...
                       row_annotation_colors = list(Cluster = hue_pal()(nlevels(Idents( custom.rna.seurat)))),
                       col_annotation_colors = list(Cluster = hue_pal()(nlevels(Idents( custom.rna.seurat)))));
...
resNULL=lapply(names(topMarkers), function(clusterName)
{
  ...
  # Extract cells from current cluster to highlight them on a dimreduc plot
  clusterCells = which( Idents( custom.rna.seurat) == clusterName);
  # Create a dimreduc plot with current cluster highlighted (useful for large number of clusters)
  print( DimPlot( custom.rna.seurat, 
                  reduction = ifelse(exists("useReduction"), useReduction, "umap"), 
                  cols="#44444422", 
  ...
  markersPanels = lapply( topMarkers[[clusterName]][["gene"]], function(featuresName) 
    { 
      FeaturePlot( custom.rna.seurat, features = featuresName, reduction = ifelse(exists("useReduction"), useReduction, "umap"), order = TRUE) +
        theme( axis.title.x = element_blank(),
               axis.title.y = element_blank(),
               legend.position = "none");
...
# Gather data to be visualized together (cell name + numID + metrics + Cluster)
cellsDataMarkers = cbind( "Cell" = colnames( custom.rna.seurat), # cell names from rownames conflict with search field in datatable
                          custom.rna.seurat[[c( "numID", "nCount_RNA", "nFeature_RNA")]],
                          "percent.mito" = as.numeric(format(custom.rna.seurat[["percent.mito", drop = TRUE]], digits = 5)),
                          "Cluster" = Idents( custom.rna.seurat));
...
for( marker_gene in topMarkersGenes){
  cellsDataMarkers = cbind( cellsDataMarkers, custom.rna.seurat@assays$RNA@data[ marker_gene, colnames( custom.rna.seurat)])
}
...
# DoHeatmap replaced by use of iHeatmapr after testing several options
#htmltools::tagList( ggplotly( DoHeatmap(object = custom.rna.seurat, features = unlist(MONITORED_GENES))));
...
resNULL = lapply( names( MONITORED_GENES), function(monitoredGroup)
{
  ...
  {
    tryCatch( FeaturePlot( custom.rna.seurat, features = featuresName, reduction = ifelse(exists("useReduction"), useReduction, "umap"), order = TRUE)  +
                theme( axis.title.x = element_blank(),
                       axis.title.y = element_blank(),
                       legend.position = "none"),
...
# Gather data to be visualized together (cell name + numID + metrics + Cluster)
cellsDataMonitored = cbind( "Cell" = colnames( custom.rna.seurat), # cell names from rownames conflict with search field in datatable
                            custom.rna.seurat[[c( "numID", "nCount_RNA", "nFeature_RNA")]],
                            "percent.mito" = as.numeric(format(custom.rna.seurat[["percent.mito", drop = TRUE]], digits = 5)),
                            "Cluster" = Idents( custom.rna.seurat));
...
for( monitored_gene in all_monitored_genes){
  cellsDataMonitored = cbind( cellsDataMonitored, custom.rna.seurat@assays$RNA@data[ monitored_gene, colnames( custom.rna.seurat)])
}
...
# ------------------------------------------------------------------
# ------------------------------------------------------------------
# MONITORED AND MARKER GENES - expression and zero pct distribution
# ------------------------------------------------------------------
# ------------------------------------------------------------------

## @knitr heterogeneity_monitoredAndMarkerGenes_stats

# Look at the relation of gene mean over cells against percentage of zero counts
# notably for marker genes, monitored genes.
# ..............................................................................
mean_pctzero_df = as.data.frame( cbind(  apply( custom.rna.seurat@assays$RNA@counts, 1, mean), 
                                         apply( custom.rna.seurat@assays$RNA@counts, 1, function( column){
                                           column = as.numeric( column)
                                           pct.zero = length( which( column == 0)) / length( column)
                                         })
))

names( mean_pctzero_df) = c( "mean", "pct.zero")
ggplot() + 
  geom_point( data = mean_pctzero_df, aes( log( mean), pct.zero), col="grey") +
  geom_point( data = mean_pctzero_df[ all_monitored_genes,], aes( log( mean), pct.zero), col="green", size = 2) +
  geom_point( data = mean_pctzero_df[ topMarkersGenes,], aes( log( mean), pct.zero), col="blue", size = 2) +
  geom_point( data = mean_pctzero_df, x=log( mean_pctzero_df[ "eYFP", "mean"]), y= mean_pctzero_df[ "eYFP", "pct.zero"], col="red", size = 3) +
  xlab( "Log of gene counts mean over cells") +
  ylab( "Percentage of gene zero counts over cells") +
  ggtitle( "Pct zero counts vs mean counts for each gene.\nBlue dots are marker genes, green dots are monitored genes, red dot is eYFP") +
  theme_classic()

# ------------------------------------------------------------------
# ------------------------------------------------------------------
# eYFP GENE - behavior of negative eYFP cells
# ------------------------------------------------------------------
# ------------------------------------------------------------------

## @knitr heterogeneity_eYFPGene_stats

# Look at some stats on the eYFP gene
# ....................................
ggplot( data = cellsDataMonitored) + 
  geom_violin( aes( x=is.eYFP, y = nCount_RNA, fill=is.eYFP)) +
  geom_jitter( aes( x=is.eYFP, y = nCount_RNA), width = 0.2, size = 0.2) +
  theme_classic() + theme( legend.position = "None")

ggplot( data = cellsDataMonitored) + 
  geom_violin( aes( x=is.eYFP, y = nFeature_RNA, fill=is.eYFP)) +
  geom_jitter( aes( x=is.eYFP, y = nFeature_RNA), width = 0.2, size = 0.2) +
  theme_classic() + theme( legend.position = "None")

ggplot( data = cellsDataMonitored) + 
  geom_violin( aes( x=is.eYFP, y = percent.mito, fill=is.eYFP)) +
  geom_jitter( aes( x=is.eYFP, y = percent.mito), width = 0.2, size = 0.2) +
  theme_classic() + theme( legend.position = "None")
