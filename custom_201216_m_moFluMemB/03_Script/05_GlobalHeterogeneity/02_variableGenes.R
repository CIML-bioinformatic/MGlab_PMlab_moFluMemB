...
custom.rna.seurat = FindVariableFeatures( object = custom.rna.seurat, 
...
variablesGenesStats = paste(length( VariableFeatures( custom.rna.seurat)), "/", nrow( custom.rna.seurat));
...
suppressMessages(suppressWarnings(LabelPoints( plot = VariableFeaturePlot( custom.rna.seurat) + theme(legend.position = "none"), 
                                               points = head( VariableFeatures( custom.rna.seurat), 10), 
                                               repel = TRUE)));
...
variableAnnotationsDT = head( HVFInfo( object = custom.rna.seurat, assay = "RNA", selection.method = 'vst')[VariableFeatures( custom.rna.seurat),], VARIABLE_FEATURES_SHOWTOP);
