...
cat("<H4>Read the data from BCR recontruction and analysis</H4>")
...
# Load the data from the file produced by blast at the end of the PMlab BCR analysis pipeline
bcr_blast_heavy_df = read.table( PATH_BCR_BLAST_HEAVYCHAIN_ANALYSIS_FILE, sep="\t", header = FALSE, stringsAsFactors = FALSE, quote='"')
names( bcr_blast_heavy_df) = c("qseqid", "sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore", "qlen", "slen")
bcr_blast_light_df = read.table( PATH_BCR_BLAST_LIGHTCHAIN_ANALYSIS_FILE, sep="\t", header = FALSE, stringsAsFactors = FALSE, quote=NULL)
names( bcr_blast_light_df) = c("qseqid", "sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore", "qlen", "slen")
...
for( line_index in 1:nrow( bcr_migmap_light_df)){
  ...
  # Get the V-segment information, removing the isoform information
...
heavy_and_light_chains_df = merge( heavy_chain_df, light_chain_df, by="cell.bc", suffixes=c( ".heavy", ".light"))
row.names( heavy_and_light_chains_df) = heavy_and_light_chains_df$cell.bc
...
heavy_and_light_chains_df$full.clonotype.similarity = paste0( heavy_and_light_chains_df$clonotype.similarity.heavy, "<<>>", heavy_and_light_chains_df$clonotype.similarity.light)
...
write.fasta( sequences = as.list( heavy_and_light_chains_df$full.trimmed.chain), names = heavy_and_light_chains_df$cell.bc,
...
clonotype_similarity_set = unique( heavy_and_light_chains_df$full.clonotype.similarity)
CLONOTYPE_GROUP_LIST = list()
CLONOTYPE_GROUP_SIZE_SET = vector()
for( clonotype in clonotype_similarity_set){
  CLONOTYPE_GROUP_LIST[[ clonotype]] = heavy_and_light_chains_df[ which( heavy_and_light_chains_df$full.clonotype.similarity == clonotype), "cell.bc"]
...
# Add the sumbol of clonotype to the global dataframe
heavy_and_light_chains_df$full.clonotype.similarity.symbol = unlist( sapply( heavy_and_light_chains_df$full.clonotype.similarity, function( name){
